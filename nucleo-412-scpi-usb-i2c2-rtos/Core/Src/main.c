...
int main(void)
{
  ...
  command_parser_init();
  SCPI_START();
  user_i2c_comms_init(&hi2c2);
  user_set_sensor_data(BME280_FORCED_MODE);
  /* USER CODE END 2 */
  ...
  while (1)
  {
		  if (Ringbuf_isDataAvailable())
		  {
			  Bufferptr = Ringbuf_getTailPointer();
			  len_to_read = Ringbuf_Readchunk();
			  scpi_instrument_write_ringbuf(Bufferptr, len_to_read);
			  Ringbuf_Confirmcopy();
		  }
